...
import dockerBuild, {
	TASK_NAME
} from '../../../../old-src/stages/deploy/tasks/dockerBuild';
...
describe('dockerBuild task', () => {
	...
	it('builds and pushes docker image, with existing match', async () => {
		...
		expect(runCommandMock).toHaveBeenNthCalledWith(
			3,
			"sudo docker image ls | grep craigmiller160.ddns.net:30004/my-project | grep 1.0.0 | awk '{ print $3 }' | xargs sudo docker image rm -f",
			{
				logOutput: true
			}
		);
