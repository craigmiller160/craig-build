...
import downloadArtifact, {
	TASK_NAME
} from '../../../../old-src/stages/deploy/tasks/downloadArtifact';
...
const mavenReleaseWorkDir = path.resolve(
	workingDirs,
	'mavenReleaseApplication'
);
const mavenReleaseBuildDir = path.resolve(
	mavenReleaseWorkDir,
	'deploy',
	'build'
);
const mavenPreReleaseWorkDir = path.resolve(
	workingDirs,
	'mavenSnapshotApplication'
);
const mavenPreReleaseBuildDir = path.resolve(
	mavenPreReleaseWorkDir,
	'deploy',
	'build'
);
...
const npmPreReleaseBuildDir = path.resolve(
	npmPreReleaseWorkDir,
	'deploy',
	'build'
);
...
describe('downloadArtifact task', () => {
	...
	it('downloads maven pre-release', async () => {
		...
		const targetPath = path.resolve(
			mavenPreReleaseBuildDir,
			'my-project-1.0.0-SNAPSHOT.jar'
		);

		expect(downloadArtifactApi).toHaveBeenCalledWith(
			downloadUrl,
			targetPath
		);
	...
	it('downloads npm beta', async () => {
		...
		const targetPath = path.resolve(
			npmPreReleaseBuildDir,
			'my-project-1.0.0-beta.tgz'
		);
