import validateDependencyVersions from '../../../old-src/stages/config-validation/tasks/validateDependencyVersions';
import validateKubeVersion from '../../../old-src/stages/config-validation/tasks/validateKubeVersion';
...
import configValidation from '../../../old-src/stages/config-validation';
...
jest.mock(
	'../../../src/stages/config-validation/tasks/validateDependencyVersions',
	() => jest.fn()
);
jest.mock(
	'../../../src/stages/config-validation/tasks/validateKubeVersion',
	() => jest.fn()
);
jest.mock('../../../src/stages/config-validation/tasks/validateGitTag', () =>
	jest.fn()
);
...
describe('configValidation stage', () => {
	...
	it('completes successfully', async () => {
		validateDependencyVersionsMock.mockImplementation(() =>
		...
		expect(validateDependencyVersionsMock).toHaveBeenCalledWith(
			projectInfo
		);
	...
	it('completes with error', async () => {
		...
		validateKubeVersionMock.mockImplementation(() =>
