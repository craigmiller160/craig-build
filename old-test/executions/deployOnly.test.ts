...
describe('deployOnly', () => {
	...
	it('runs successfully', async () => {
		...
		expect(buildLogger).toHaveBeenNthCalledWith(
			1,
			'Deploying only undefined'
		);
		expect(buildLogger).toHaveBeenNthCalledWith(
			2,
			'Deploy only finished successfully',
			'success'
		);
	...
	it('logs build error', async () => {
		...
		expect(buildLogger).toHaveBeenNthCalledWith(
			2,
			'Deploy only failed on Stage TheStage and Task TheTask: Error',
			'error'
		);
