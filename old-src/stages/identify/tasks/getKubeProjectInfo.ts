...
const findKubeProjectInfo = (
	projectInfo: ProjectInfo
): E.Either<Error, ProjectInfo> =>
	E.tryCatch(() => {
		const kubeDeploymentPath = path.resolve(
			getCwd(),
			'deploy',
			'deployment.yml'
		);
		...
		const kubeDeployment = yaml.parse(
			kubeDeploymentContent
		) as KubeDeployment;

		const craigAppContainerIndex =
			kubeDeployment.spec.template.spec.containers.findIndex(
				(container) => container.image.includes('craigmiller160')
			);
		if (craigAppContainerIndex >= 0) {
			return {
				...
				kubernetesDockerImage:
					kubeDeployment.spec.template.spec.containers[
						craigAppContainerIndex
					].image
			};
		}
		return projectInfo;
...
const getKubeProjectInfo: TaskFunction<ProjectInfo> = (
...
export default createTask(stageName, TASK_NAME, getKubeProjectInfo, [
