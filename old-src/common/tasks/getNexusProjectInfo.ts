...
const updateNexusProjectInfo = (
	projectInfo: ProjectInfo,
	releaseResult: NexusSearchResult,
	preReleaseResult?: NexusSearchResult
): ProjectInfo => {
	const preReleaseVersion =
		(preReleaseResult?.items.length ?? 0) > 0
			? preReleaseResult?.items[0].version
			: undefined;
	const releaseVersion =
		releaseResult.items.length > 0
			? releaseResult.items[0].version
			: undefined;
	return {
		...projectInfo,
		latestNexusVersions: {
...
const lookupDockerNexusVersions = (
	projectInfo: ProjectInfo
...
const lookupMavenNexusVersions = (
	projectInfo: ProjectInfo
): TE.TaskEither<Error, ProjectInfo> =>
	pipe(
		searchForMavenReleases(projectInfo.group, projectInfo.name),
		TE.chain((releaseResult: NexusSearchResult) =>
			pipe(
		...
		default:
			return TE.left(
				context.createBuildError(
					`Invalid ProjectType for finding version info: ${context.input.projectType}`
				)
			);
	}
};

const getNexusProjectInfo: TaskFunction<ProjectInfo> = (
	context: TaskContext<ProjectInfo>
) =>
	pipe(
...
export default (stageName: string) =>
	createTask(stageName, TASK_NAME, getNexusProjectInfo);
