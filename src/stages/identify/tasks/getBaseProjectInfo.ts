...
import PackageJson, {
	Dependencies as NpmDependencies
...
const getMavenDependencies = (pomXml: PomXml): Dependency[] => {
	const propertyMap: KeyValueMap = Object.entries(
		pomXml.project.properties[0]
	...
	if (
		...
		pomXml.project.dependencies.length === 0
	) {
		return [];
	}

	return pomXml.project.dependencies[0].dependency.map((dependency) => {
		let version = dependency.version?.[0] ?? '';
		if (version.startsWith('${')) {
			const mapKey = version.replace(/^\${/, '').replace(/}$/, '');
...
const getBaseProjectInfo: TaskFunction<ProjectType, ProjectInfo> = (
	context: TaskContext<ProjectType>
) =>
	pipe(
