...
const whichVersionToBump = (
	context: TaskContext<ProjectInfo>,
	nexusVersion: string
): E.Either<Error, string> => {
	const comparison = semver.compare(
		trimVersion(context.input.version),
		trimVersion(nexusVersion)
	);
	if (comparison === 1) {
		...
		return E.left(
			context.createBuildError(
				`Nexus beta version cannot be higher than project version. Project: ${context.input.version} Nexus: ${nexusVersion}`
			)
		);
	}
