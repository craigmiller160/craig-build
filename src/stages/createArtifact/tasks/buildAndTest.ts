...
const mavenBuildAndTest = (
	context: TaskContext<ProjectInfo>
): TE.TaskEither<Error, ProjectInfo> => {
	context.logger(
		'Maven will automatically perform publish step if build successful'
	);
	return pipe(
...
const doBuildAndTest = (
	...
	switch (context.input.projectType) {
		...
		default:
			return TE.left(
				new Error(`Invalid project type: ${context.input.projectType}`)
			);
	}
};

const buildAndTest: TaskFunction<ProjectInfo> = (
	context: TaskContext<ProjectInfo>
) =>
	pipe(
