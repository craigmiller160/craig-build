	...
	pipe(
		...
		TE.chain((nexusSearchResult) => {
			if (nexusSearchResult.items.length > 0) {
				return TE.right(
					nexusSearchResult.items[0].assets[0].downloadUrl
				);
			}
			return TE.left(
				context.createBuildError(
					`Unable to find artifact in Nexus. ` +
...
const doDownloadArtifact = (
	...
	return TE.left(
		context.createBuildError(
			`Invalid project for downloading artifact: ${JSON.stringify(
				context.input
			)}`
		)
	);
...
const downloadArtifact: TaskFunction<ProjectInfo> = (
	context: TaskContext<ProjectInfo>
) => {
...
export default createTask(stageName, TASK_NAME, downloadArtifact, [
	executeIfApplication,
	executeIfNotDocker
]);
