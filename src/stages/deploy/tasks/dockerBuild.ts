...
const createDockerPush = (tag: string) => `sudo docker push ${tag}`;
...
const removeExistingDockerImages = (
		...
		O.fold(
			() => {
				if (isDocker(projectInfo.projectType)) {
					return O.some(
						`${DOCKER_REPO}/${projectInfo.name}:${projectInfo.version}`
					);
				}
				return O.none;
			},
...
const dockerBuild: TaskFunction<ProjectInfo> = (
	...
	return pipe(
		...
		E.chain((dockerImage) =>
			pipe(
				runCommand(
					createDockerLogin(NEXUS_DOCKER_USER, NEXUS_DOCKER_PASSWORD),
					{ logOutput: true }
				),
				E.map(() => dockerImage)
			)
		...
		E.chain((dockerImage: string) =>
			pipe(
				runCommand(createDockerBuild(dockerImage), {
					cwd: deployDir,
					logOutput: true
				}),
				E.map(() => dockerImage)
		...
		E.chain((dockerImage: string) =>
			pipe(
				runCommand(createDockerPush(dockerImage), {
					cwd: deployDir,
					logOutput: true
				}),
				E.map(() => dockerImage)
