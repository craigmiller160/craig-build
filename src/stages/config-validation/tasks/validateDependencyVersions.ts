...
const validateDependencies = (
	...
	return pipe(
		...
		A.reduce<Dependency, E.Either<string, ProjectInfo>>(
			E.right(projectInfo),
			(result, dependency) => {
				let message = '';
				if (isLeft(result)) {
		...
		E.mapLeft((errorMessage: string) => {
			return context.createBuildError(
				`${preReleaseFlag} dependencies not allowed in release build: ${errorMessage}`
			);
		})
	);
};

const doVersionValidation = (
	context: TaskContext<ProjectInfo>
): E.Either<Error, ProjectInfo> => {
	switch (context.input.projectType) {
		...
		case ProjectType.MavenLibrary:
			return validateDependencies(
				MAVEN_CRAIG_DEP_PREFIX,
				MAVEN_PRE_RELEASE_FLAG,
				context
			);
		case ProjectType.NpmApplication:
		case ProjectType.NpmLibrary:
			return validateDependencies(
				NPM_CRAIG_DEP_PREFIX,
				NPM_PRE_RELEASE_FLAG,
				context
			);
...
const validateDependencyVersions: TaskFunction<ProjectInfo> = (
	context: TaskContext<ProjectInfo>
) =>
	pipe(
...
export default createTask(stageName, TASK_NAME, validateDependencyVersions, [
	executeIfRelease
]);
