...
export type TaskShouldExecuteFunction<Input> = (context: TaskContext<Input>) => string | undefined;
export interface Task<Input,ResultValue> {
    operation: (input: Input) => TE.TaskEither<Error, ResultValue>;
    shouldExecute: (context: TaskContext<Input>) => string | undefined;
}

const defaultShouldExecute: TaskShouldExecuteFunction<any> = (context: TaskContext<any>) => undefined;

// TODO when tasks are added to a stage, ensure they are of the Task type

const createTask = <Input, ResultValue>(stageName: string, taskName: string, taskFn: TaskFunction<Input, ResultValue>, shouldExecuteFn?: TaskShouldExecuteFunction<Input>): Task<Input,ResultValue> => ({
    shouldExecute: (context: TaskContext<Input>) => {
        return shouldExecuteFn ? shouldExecuteFn(context) : defaultShouldExecute(context);
    },
    operation: (input: Input): TE.TaskEither<Error, ResultValue> => {
