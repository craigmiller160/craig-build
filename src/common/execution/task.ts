...
export type TaskFunction<Input, ResultValue = Input> = (
	context: TaskContext<Input>
) => TE.TaskEither<Error, Result<ResultValue>>;
export type TaskShouldExecuteFunction<Input, ResultValue = Input> = (
	input: Input
) => TaskSkipExecutionResult<ResultValue> | undefined;
export type BuildTask<Input, ResultValue = Input> = (
	input: Input
...
const createTask =
	<Input, ResultValue = Input>(
		...
		shouldExecuteFns: TaskShouldExecuteFunction<Input, ResultValue>[] = []
	): BuildTask<Input, ResultValue> =>
	(input: Input): TE.TaskEither<Error, ResultValue> => {
		...
		return pipe(
			...
			O.map((result: TaskSkipExecutionResult<ResultValue>) => {
				taskContext.logger(
					`Skipping task ${taskName}: ${result.message}`
				);
